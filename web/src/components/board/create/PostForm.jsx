...
export default function PostForm({
    ...
    return (<form id="create-post-form" onSubmit={handleSubmit} className="p-4 space-y-4">
            <div className="text-sm">
                <div className="flex"><span className="w-16 font-semibold text-gray-600">작성자</span><span>Jueon</span>
                </div>
                <div className="flex mt-1"><span
                    className="w-16 font-semibold text-gray-600">작성날짜</span><span>{today}</span></div>
            </div>
            <div className="grid grid-cols-3 gap-2.5">
                <div className="relative"><select onChange={handleCountryChange}
                ...
                <div className="relative"><select name="locationId" value={postData.locationId}
                                                  onChange={handleInputChange}
                                                  className="w-full appearance-none rounded-md border border-gray-300 bg-white p-2.5 pr-8 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">{availableRegions.map(region =>
                    <option key={region.name} value={region.id || ''}>{region.name}</option>)}</select>
                    <div
                        className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 text-gray-500">
                        <ChevronDown size={16}/></div>
                </div>
                <div className="relative"><select name="categoryId" value={postData.categoryId}
                                                  onChange={handleInputChange}
            ...
            <textarea name="content" value={postData.content} onChange={handleInputChange} placeholder="내용을 입력해주세요."
                      className="w-full h-48 rounded-lg border border-gray-300 p-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"/>
            <div><input type="file" multiple hidden ref={fileInputRef} onChange={handleImageUpload} accept="image/*"/>
            ...
            {images.length > 0 && (<div className="grid grid-cols-3 gap-2">{images.map(image => (
                <div key={image.id} className="relative aspect-square"><img src={image.previewUrl} alt="미리보기"
                                                                            className="h-full w-full rounded-md object-cover"/>
                    <button type="button" onClick={() => handleImageDelete(image.id)}
