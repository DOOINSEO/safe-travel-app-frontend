...
export default function CommentSection({comments, setComments, currentUserId}) { // currentUserId prop 추가
    ...
    const handleSaveEdit = (commentId) => {
        setComments(prevComments => prevComments.map(c => c.commentId === commentId ? {
            ...c,
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex justify-end space-x-2">
                                        <button onClick={handleCancelEdit}
                                                className="rounded-md px-3 py-1 text-sm text-gray-600 hover:bg-gray-100">취소
                                        </button>
                                        <button onClick={() => handleSaveEdit(comment.commentId)}
                                                className="rounded-md bg-blue-500 px-3 py-1 text-sm text-white hover:bg-blue-600">저장
                                        </button>
                                    </div>
                                </div>) : (// --- 일반 모드 UI ---
                                <div className="flex items-start space-x-3">
                                    <div className="flex-1 rounded-lg bg-gray-100 p-3">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <span
                                                className="text-xs text-gray-500">{formatDate(comment.createdAt)}</span>
                                        </div>
                                        <p className="mt-1 text-sm text-gray-800">{comment.content}</p>
                                    </div>
                                    {/* --- 수정된 부분: currentUserId와 comment.userId 일치 여부에 따라 버튼 표시 --- */}
                                    {comment.userId === currentUserId && (<div className="flex flex-col space-y-1">
                                            <button onClick={() => handleStartEdit(comment)}
                                                    className="p-1 text-gray-400 hover:text-blue-500"
                                                    aria-label="댓글 수정"><Pencil size={16}/></button>
                                            <button onClick={() => setCommentToDeleteId(comment.commentId)}
                                                    className="p-1 text-gray-400 hover:text-red-500" aria-label="댓글 삭제">
