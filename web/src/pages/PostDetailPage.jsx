...
import {DUMMY_COMMENTS, DUMMY_POSTS} from '../data/dummyData';

const DUMMY_LOCATIONS = [{
    country: '캄보디아',
    id: 1,
    regions: [{name: '시엠레아프', id: 101}, {name: '프놈펜', id: 102}]
}, {country: '터키', id: 2, regions: [{name: '이스탄불', id: 201}, {name: '앙카라', id: 202}]},];
const DUMMY_CATEGORIES = [{id: 1, name: '기상이변'}, {id: 2, name: '소매치기'},];
...
export default function PostDetailPage() {
    ...
    if (!post) {
        return (<div className="min-h-screen bg-white"><PageHeader title="오류"/>
            <div className="flex flex-col items-center justify-center pt-20 text-center"><Frown
                className="h-16 w-16 text-gray-400"/><h2 className="mt-4 text-xl font-semibold">게시글을 찾을 수 없습니다.</h2><p
                className="mt-2 text-gray-500">삭제되었거나 잘못된 경로입니다.</p></div>
        </div>);
    }

    const isOwner = post.userId === CURRENT_USER_ID;

    const handleLikeToggle = () => setPost(prev => ({
        ...prev,
        isLike: !prev.isLike,
    ...
    const handleSaveEdit = () => {
        const finalImages = editedImages.map((img, i) => (img.file ? {
            imageId: Math.random(),
            postId: post.postId,
    ...
    const handleCountryChangeForEdit = (e) => {
        ...
        const data = DUMMY_LOCATIONS.find(c => c.country === name);
    ...
    return (<div className="flex h-screen flex-col bg-white">
            ...
            <main className="flex-1 overflow-y-auto">
                <div className="p-4">
                    ...
                    <PostContentView post={post} comments={comments} isEditing={isEditing} isOwner={isOwner}
                                     editedState={editedState} editHandlers={editHandlers} fileInputRef={fileInputRef}/>
            ...
            <footer className="border-t bg-white p-2">
                <form onSubmit={handleCommentSubmit} className="flex items-center space-x-2"><input type="text"
                                                                                                    value={newComment}
                    ...
                    <button type="submit"
                            ...
                            disabled={!newComment.trim()} aria-label="댓글 전송"><Send size={18}/></button>
                </form>
            </footer>
            <ConfirmModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)}
                          onConfirm={handleDeletePost} title="게시물 삭제 확인" intent="destructive" confirmText="삭제">정말로 이
                게시물을 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.</ConfirmModal>
        </div>);
}